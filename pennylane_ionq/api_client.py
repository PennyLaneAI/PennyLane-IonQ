...
class ResourceManager:
    ...
    def _warn_memory_fallback(self, detail):
        ...
        warnings.warn(
            "You set the memory argument `True` on the device. "
    ...
    def _retrieve_child_job_shots(self, child_job_ids, params=None):
        ...
        for job_id in child_job_ids:
            shots_by_child[job_id] = None
            try:
                response = self.client.get(self.join_path(str(job_id)), params=params)
                response.raise_for_status()
            except requests.exceptions.RequestException as ex:
                self._warn_memory_fallback(
                    f"However, the request to retrieve shots for child {job_id} failed on getting child job data with exception: {ex}."
                )
                continue
            ...
            if isinstance(url, str) and url:
                try:
                    ...
                    self._warn_memory_fallback(
                        f"However, the request to retrieve shots for child {job_id} failed on getting shots data with exception: {ex}."
                    )
                    continue
                shots_by_child[job_id] = response.json()
            else:
                self._warn_memory_fallback(
                    f"However, no shots results URL was found in IonQ response for child job {job_id}."
                )
    ...
    def refresh_data(self, data, params=None, fetch_shots=False):
        ...
        supports_shot_payload = hasattr(self.resource, "shots") and hasattr(
            self.resource, "has_shots"
        )

        if supports_shot_payload:
            self.resource.shots = None
            self.resource.has_shots = False

            if fetch_shots:
                self.resource.has_shots = True
                child_job_ids = data.get("child_job_ids")
                if child_job_ids:
                    self.resource.shots = self._retrieve_child_job_shots(
                    ...
                    if isinstance(url, str) and url:
                        try:
                            ...
                            self._warn_memory_fallback(
                                f"However, the request to retrieve shots for this job failed on getting shots data with exception: {ex}."
                            )
                        else:
                            shot_data = resp.json()
                    else:
                        self._warn_memory_fallback(
                            "However, no shots results URL was found in IonQ response for this job."
                        )

                    self.resource.shots = shot_data
